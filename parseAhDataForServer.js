...
function update(realm, region, callback)
{
	getData.updateServerAuctionData(realm, region, (filename) => {
		...
		console.log("Log updated for "+realm+"-"+region+" with filename: "+filename);
...
exports.queryForItem = function(itemName, realm, region, callback)
{
	update(realm, region, (obj) => {
		...
		if (id === undefined)
		{
			callback("Sorry the requested item couldn't be found, please check your spelling and try again (not case sensitive).");
			return;
		} 
		...
		for (i = 0; i < filteredItems.length; i++) // going over only those lines with id
		{
			var auction = filteredItems[i];
			var pricePerItem = Math.round(auction.buyout / auction.quantity);
			items.push([auction.item, pricePerItem, auction.quantity, auction.buyout, auction.owner])
		}
	 	...
	 	for (key in count(items))
	  	{
	  		...
	  		if (Number(keyArrayValues[3]) === 0){continue;}
	  		if (num % 2 ===  0)
	  		{
		  		stringArray.push(["."+count(items)[key],formatCurrency(keyArrayValues[1]),keyArrayValues[2],formatCurrency(keyArrayValues[3]),keyArrayValues[4]]);
	  		}else{
		  		stringArray.push(["`"+count(items)[key],formatCurrency(keyArrayValues[1]),keyArrayValues[2],formatCurrency(keyArrayValues[3]),keyArrayValues[4]+"'"]);
	  		}
	  		num++;
...
exports.listAllItemsOfOwner = function(owner, realm, region, callback)
{
	update(realm, region, (obj) => {
		...
		var stringArray = [];
	 	...
	 	{
	 		if(num > 50){conditionallyNumbersome = "50/"; break;}
	 		if (num % 2 ===  0)
	 		{	
	 			value = finalDict[key];
	  	...
	  	callback("```AsciiDoc\n"+"["+owner+" "+obj.realms[0].name+"'s posts ("+conditionallyNumbersome+Object.keys(finalDict).length+" items):]"+"\n"+t+"\n```");
	  	//callback(finalDict); 
	});
}
...
exports.undercut = function(owner, itemName, realm, region, callback)
{
	...
	update(realm, region, (obj) => {
		...
		if (itemName !== "")
		{
			...
			filteredItems = where(obj.auctions, {"owner":name, "item":Number(getItemFromCache(itemName))});
		}else{
			filteredItems = where(obj.auctions, {"owner":name});
		}

		if(filteredItems.length === 0){
			callback('Couldn\'t find that person/item, check your spelling or try a different person/item.');
			return;
		}
		...
		for(var i = 0; i < Object.keys(filteredItems).length; i++)
		{
			...
			var pricePerItem = Math.round(filteredItems[i].buyout / filteredItems[i].quantity);
			...
			var undercut = _filter(obj.auctions, function (o) {
				return ( (o.item === item) && (o.owner !== owner) && (o.buyout !== 0) && (Math.round(o.buyout / o.quantity) < pricePerItem) && (o.quantity === quantity) );
			});
			...
			if (undercut.length > 0){
				...
				for(var k = 0; k < undercut.length; k++){
					var oPricePerItem = Math.round(undercut[k].buyout / undercut[k].quantity);
		...
		var conditionallyNumbersome = "";
	 	...
	 	{
	 		if(v > 40){ conditionallyNumbersome = "40/"; break;}
	 		if (v % 2 ===  0)
	 		{	
  		...
  		callback("```AsciiDoc\n"+"["+name+" "+obj.realms[0].name+"'s list of "+conditionallyNumbersome+newList.length+" undercut item(s):]"+"\n"+t+"\n```");
	});
}
...
exports.topPosters = function(realm, region, callback)
{
	update(realm, region, (obj) => {
		var dict = {};
		for(i = 0; i < Object.keys(obj.auctions).length; i++) //Object.keys(obj.auctions).length  // Goes through every auction item
		{
		...
		items = items.slice(0, 10);
	  	...
	  	callback("```AsciiDoc\n"+"[Top posters for "+obj.realms[0].name+" are:]"+"\n"+t+"\n```");
 	});
}
...
exports.uniqueAuctionItems = function(realm, region, callback)
{
	update(realm, region, (obj) => {
		var dict = {};
		for(i = 0; i < Object.keys(obj.auctions).length; i++) // Goes through every auction item
		{
	...
	if (fs.existsSync('itemNames.json'))
        {
            ...
            for(i = 0; i < Object.keys(object).length; i++) // search if we have this object already
            {
                if (object[i].name.toLowerCase() === itemName.toLowerCase()) // we have this item already, let's return it
                {
                    id = object[i].id;
