...
{ 
	...
	r.getJSON(options,
	    function(statusCode, result)
	    {
		    ...
		    getRealmAuctionData(options, realm, region, match, writeUpdate, (filename) => {
		    	onFinish(filename);
		    });
		});
}// module end
...
function setOptions(realm, region, apikey)
{
	...
	if (region !== 'en_US')
	{
		options = {
		    ...
		    port: 443,
		    path: '/wow/auction/data/'+ realm +'?locale='+ region +'&apikey='+ apiKey,
		    method: 'GET',
		    headers: {
...
function realmNeedsUpdate(url,lastMod)
{
	if (fs.existsSync('lastUpdate.json'))
	{
		...
		{
			if (object[i].files[0].url === url) // found a match
			{
...
function getRealmData(obj, goTo)
{
	...
	if (fs.existsSync('lastUpdate.json'))
	{
		fs.readFile('lastUpdate.json', 'utf8', function readFileCallback(err, data){
			if (err){
				console.log(err);
			}else{
				// PLACEHOLDER FOR var match = false;
				var object = JSON.parse(data);
				for(i = 0; i < Object.keys(object).length; i++) // search if we have this item already
				{
					if (object[i].files[0].url === goTo) // found a match
					{
						match = true;
	...
	}else{ // There is no lastUpdate.json file, let's make it!
...
function getRealmAuctionData(options, realm, region, match, writeUpdate, callback)
{
     //subsequent call to get the big blob
    r.getJSON(options,
		function(statusCode, finalResult)
		{
			var filename = 'AH_DATA_' + realm +'_'+ region + '.json';
			var ahBlob = finalResult;
			console.log(statusCode);
			if ( (statusCode === 200 && match === false) || (statusCode == 200 && writeUpdate === true) ){
