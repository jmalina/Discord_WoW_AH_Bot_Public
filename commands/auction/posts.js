...
class ccPosts extends Commando.Command {
  constructor(client) {
    super(client, {
      ...
      memberName: "posts",
      description:
        'How many posts a person has on the auction house, adding "all" will list items',
      details:
        'Adding "all" after the name will display a list of unique items that user has posted. Doing $posts with nothing else shows how many posts are on that realm.',
      examples: [
        "$posts Cc all Ysondre US, $posts dade arthas eu, $posts budvar, $posts cc all, $posts",
      ],
      argsCount: 2,
      argsType: "multiple",
    });
        ...
        if (name === "" || name === undefined) {
          ahData.numOfAuctions(realm, region, function callback(data) {
            ...
            msg.message.channel.sendMessage(
              `There are ${data} posts on this realms auction house`
            );
          });
        } else if (text.toLowerCase() === "all") {
          ahData.listAllItemsOfOwner(
            name,
            realm,
            region,
          ...
          ahData.numOfAuctionsForOwner(
            ...
            function callback(data) {
              message.delete();
              msg.message.channel.sendMessage(
                `${name} ${realm}-${region} has ${data} posts on the auction house`
              );
            }
          );
        }
