...
class ccUndercut extends Commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'auction',
            memberName: 'undercut',
            description: 'Shows a persons items that are undercut. Adding an item just shows data about if that item is undercut.',
            details: 'Duplicates in the list indicate items posted at different prices or stack sizes',
            examples: ['$uc dade arthas eu, $undercut cc, $undercut cc Potion of the Old War ysondre us, $undercut cc dawnlight'],
            argsCount: 2,
            argsType: 'multiple'
        });
    }
    ...
    async run(msg, args)
    {
        ...
        if(args.length < 1)
        {
            msg.message.channel.sendMessage('Examples: $uc dade arthas eu OR $undercut cc OR $undercut cc Potion of the Old War ysondre us OR $undercut cc dawnlight');
            return;
        }
